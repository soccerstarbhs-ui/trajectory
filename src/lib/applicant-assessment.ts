...
function compare(dimension: BenchmarkComparison["dimension"], hours: number, benchmark: AdmissionsBenchmark["clinical"], schoolCount: number): BenchmarkComparison {
  ...
  return { dimension, hours, band, benchmark: { p10: benchmark.p10, p25: benchmark.p25, median: benchmark.median }, coverage: benchmark.coverage.median, note: `${text} Self-reported median coverage: ${benchmark.coverage.median}/${schoolCount} schools; use directionally.` };
}
...
export function deriveApplicantAssessment(snapshot: ApplicantProfileSnapshot, asOfDate = new Date()): ReadinessAssessment {
  ...
  return {
    ...
    coursework,
    targetFit,
    comparisons: { clinical: compare("clinical", clinicalHours, benchmark.clinical, benchmark.schoolCount), service: compare("service", serviceHours, benchmark.nonclinicalService, benchmark.schoolCount), research: compare("research", researchHours, benchmark.research, benchmark.schoolCount) },
  };
}
...
export function buildPersonalizedActionNodes(snapshot: ApplicantProfileSnapshot, assessment = deriveApplicantAssessment(snapshot)): EngineNode[] {
  ...
  const activityEvidence = `${benchmarkSources.activities.note} The selected tier includes ${targetFit.benchmark.interviewRecordCount.toLocaleString()} school-interview records across ${targetFit.benchmark.interviewRecordCoverage} reporting schools; records are not necessarily unique applicants.`;
  if (snapshot.basic.mcatStatus !== "completed" || (targetFit.mcatPercentileEstimate ?? 100) < 25) nodes.push(actionNode("mcat", snapshot.basic.mcatStatus === "completed" ? "Build a focused MCAT retake decision plan" : `Set and execute a ${targetFit.suggestedMcatGoal}+ MCAT plan`, "testing", `Use ${targetFit.suggestedMcatGoal} as a planning target for ${targetFit.benchmark.shortLabel}, then adjust around practice-test performance and timing.`, { weekly_hours: 10, base_priority: 0.95, evidence_type: "school_data", evidence_note: benchmarkSources.academic.note }));
  ...
  else if (!assessment.researchDepth.hasOutput && (["t10", "t20"] as TargetTier[]).includes(targetFit.targetTier)) nodes.push(actionNode("research-output", "Turn existing research into a poster, abstract, or manuscript", "research", `Self-reported profiles show publications frequently among selective targets (${Math.round(targetFit.benchmark.publicationShare * 100)}% across ${targetFit.benchmark.publicationCoverage} reporting schools). This is descriptive, not a requirement.`, { weekly_hours: 4, base_priority: 0.88, evidence_type: "self_reported", evidence_note: activityEvidence, deepen_existing: true }));
  ...
  if (targetFit.status === "reach" || targetFit.status === "statistically_improbable") nodes.push(actionNode("school-list", "Rebalance your school list using school-specific ranges", "planning", targetFit.message, { weekly_hours: 2, base_priority: targetFit.status === "statistically_improbable" ? 1 : 0.82, evidence_type: "school_data", evidence_note: benchmarkSources.academic.note }));
